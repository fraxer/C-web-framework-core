...
**Сделано (четвёртый срез) — проводка. Соединение живо.**

Демультиплексор создаёт соединения из Initial, маршрутизирует датаграммы по
CID, разбирает очередь отправки и стареет соединения на тике воркера.

Проверено настоящим обменом (`h3test/initial.py` — независимая реализация
key schedule RFC 9001 на Python, так что совпадение означает правоту обеих
сторон, а не одну ошибку, сделанную дважды): сервер принял Initial, создал
соединение, расшифровал пакет, обработал PING и вернул **расшифровываемый
ACK**. 30 соединений подряд, все проверки на некорректный ввод из фазы 1
по-прежнему проходят, ASan при завершении молчит.

### Два решения в connection-слое

Соединение QUIC **встраивает** `connection_t` первым полем — иначе приведение
между ними невозможно, а весь верхний слой написан против `connection_t`.
Отсюда две правки, обе аддитивные:

- `connection_s_init()` — инициализация в память вызывающего;
  `connection_s_alloc()` стал обёрткой над ней.
- `connection_server_ctx_t::transport_free` — хук, вызываемый из
  `__ctx_free`. Состояние QUIC обязано освобождаться ровно в момент
  освобождения соединения, а знает этот момент только `connection_free`.
  Хук не освобождает сам объект: `free(connection)` и есть освобождение
  `quicconn_t`, потому что `conn` лежит по нулевому смещению.

Рефакторинг проверялся отдельно, до всего остального: 100 156 проверок
TCP-сборки зелёные.

### Что в фазе 4 осталось за скобками

Написано и работает всё, кроме того, что осознанно отложено: миграция и
валидация пути (§9), key update (§6), выдача `NEW_CONNECTION_ID`, Retry по
порогу нагрузки. Каждое помечено в коде и относится к фазе 9 либо к моменту,
когда появится чем это проверить.

### Рукопожатие проверено вживую

Написан тестовый клиент (`tests/quicclient/`, цель `quicclient`) — он
переиспользует кодеки сервера и `quictls_init_client`, своей является только
клиентская половина пакетного цикла. Результат:

```
handshake complete:        yes
HANDSHAKE_DONE received:   yes
ALPN:                      h3
cipher:                    TLS_AES_128_GCM_SHA256
```

20 рукопожатий подряд, ASan чист. Проверки фазы 1 (мусор, VN, stateless reset)
и обмен PING/ACK продолжают проходить.

### Три ошибки, найденные этой проверкой

Ни одну из них не показали бы юнит-тесты — все три живут на стыке модулей.

**1. SNI-колбэк падал на QUIC-соединении.** Он берёт `connection_t` из
`SSL_get_app_data`, которого у QUIC-`SSL` не было, и разыменовывал NULL.
Обработчик сигналов сервера перехватывал SIGSEGV раньше ASan, поэтому в логах
было пусто, а процесс просто исчезал — нашлось через gdb.

**2. Глубже: у vhost должно быть ДВА `SSL_CTX`.** SNI-колбэк переключает
контекст на лету, и для QUIC-соединения он переключал бы на TCP-контекст — с
`h2` в ALPN и CCM в списке шифров, который RFC 9001 §5.3 запрещает. Получилось
бы работающее и при этом несоответствующее соединение, то есть худший вид
ошибки. Добавлен `openssl_t::quic_ctx`, настраиваемый через
`quictls_configure_ctx`; колбэк выбирает контекст по `connection->transport`.

**3. Use-after-free: таблица CID создавалась с `acquire = NULL`.**
`lookup_acquire` не брал ссылку, а `__dispatch` её снимал, поэтому соединение
освобождалось после первой же полученной датаграммы. **Рукопожатие при этом
проходило целиком** — на памяти в карантине ASan. Без ASan это выглядело бы
как «иногда не приходит HANDSHAKE_DONE».

Клиент нашёл и свою ошибку: он отбрасывал Handshake-пакет, пришедший
скоалесцированным с Initial, потому что вызывал `quictls_advance` только после
всей датаграммы, а ключи уровня появляются в середине неё.
