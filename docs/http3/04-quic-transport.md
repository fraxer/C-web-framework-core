...
**Осталось:** `quicconn` — сборка соединения (приём пакета: расшифровка,
фреймы, диспетчеризация; сборка пакета: pacing, cwnd, anti-amplification;
управление CID, закрытие, idle timeout) и интеграция с эндпоинтом
(`connection_t::transport`, обход `epoll_ctl`, создание соединения из Initial).
Это вся интеграционная часть фазы; всё, что ниже неё, готово и покрыто тестами.
