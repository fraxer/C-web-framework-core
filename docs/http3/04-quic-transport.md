...
Интерфейс виртуальный (структура с указателями на функции): NewReno, CUBIC и
BBR переключаются без правок в `quicloss`. Шов держал два первых без единой
правки; BBR потребовал одно добавление — `on_ack_end`, вызываемый один раз на
подтверждение, когда все его пакеты обработаны и объявленные им потери
посчитаны. Причина в том, что скорость меряется на интервале, а поштучный
колбэк интервала не видит:
...
typedef struct quiccc_ops {
    ...
    /* Одно подтверждение целиком: скорость доставки, что подтверждено, что
     * объявлено потерянным, сколько было в полёте. Окно-ориентированные
     * контроллеры здесь ничего не делают. */
    void (*on_ack_end)(quiccc_t*, const quiccc_sample_t*, uint64_t now_us);
} quiccc_ops_t;
```
...
Третья — **BBR** (draft-cardwell-iccrg-bbr-congestion-control), ключ
`http3_cc = "bbr"`. Она отвечает не на вопрос «сколько байт держать в полёте», а
на вопрос «с какой скоростью их отдавать», и потому устроена иначе всех
остальных:

- модель пути это два измерения — `BtlBw` (максимум скорости доставки за
  последние 10 round trip'ов) и `RTprop` (минимум RTT за последние 10 секунд).
  Оба берутся как экстремумы, потому что очередь способна раздуть среднее и не
  способна раздуть ни максимум скорости, ни минимум задержки. Их произведение —
  сколько байт держит путь; всё сверх этого лежит в очереди;
- скорость доставки меряет `quicloss` по правилам
  draft-cheng-iccrg-delivery-rate-estimation: `quicsent_t` несёт снимок
  счётчика доставленного на момент отправки, и сэмпл берётся из последнего
  отправленного среди подтверждённых пакетов. Сэмплы, взятые при сендере без
  данных, помечаются `app_limited` и не могут **понизить** оценку;
- STARTUP удваивает скорость каждый round trip и заканчивается, когда три
  подряд не подняли её на 25%; DRAIN отдаёт созданную очередь; PROBE_BW ходит
  циклом из восьми фаз (5/4, 3/4, шесть по 1), проверяя, не появилось ли больше
  полосы; PROBE_RTT раз в десять секунд опускает окно до четырёх датаграмм на
  200 мс, потому что минимальный RTT видно только на пустом пути;
- **потеря не является сигналом перегрузки**: окно уменьшается на потерянные
  байты и на один round trip включается packet conservation, а модель
  продолжает строиться из измеренной скорости. Это и есть разница, ради которой
  BBR существует: на канале с фоновыми потерями NewReno и CUBIC складывают окно,
  не имея на то причины;
- пейсер здесь **первичен**: `quiccc_t.pacing_rate` задаётся моделью
  (`pacing_gain · BtlBw`) и перекрывает формулу §7.7 ниже, а `cwnd` остаётся
  границей безопасности. Поэтому `http3_pacing: false` вместе с `bbr`
  отвергается при загрузке конфигурации.
...
Формат — JSON-SEQ (`.sqlog`), одна строка на событие. Включается конфигом на N
первых соединений, иначе на нагрузке это само по себе DoS. Готовые визуализаторы
(qvis) принимают этот формат и рисуют диаграммы cwnd/потоков — это и есть
основной инструмент отладки восстановления.

**Сделано 2026-08-18** (`08` §10). Что вышло иначе, чем написано выше:

* **Строки собираются `snprintf`, а не `misc/json.h`.** Событие — это одна
  строка фиксированной формы, её генератор объектов строит дольше, чем стоит
  само событие, и делает это на пакетном пути. Цена решения — экранирование
  чужих байтов (причина `CONNECTION_CLOSE`, ALPN) стало обязанностью вызывающего:
  для этого есть `quicqlog_escape`, и всё, что выбрал пир, идёт через него.
  Кавычка или перевод строки внутри события портит не поле, а весь файл.
* **Выбор соединений — только «первые N», без префикса CID.** Префикс полезен,
  когда известно, какое соединение ловить; в инцидентах, ради которых это
  писалось, известно обратное — что «какое-то из них». Ключ остаётся местом,
  куда префикс добавляется, если понадобится.
* **Гейт рантаймовый, а не сборочный.** `CWFR_QUIC_QLOG` убран: инцидент
  случается на уже собранном и запущенном сервере, а инструмент, требующий
  пересборки, отвечает после инцидента. Цена — проверка указателя на каждом
  месте вызова; включение стоит `http3_qlog_dir` и reload'а.
* **Запись без буфера.** Журнал, теряющий хвост, бесполезен ровно в том случае,
  ради которого его чаще всего открывают: соединение зависло, и ответ — в
  последних событиях перед тишиной.
* Сверх минимального набора добавлены `transport:parameters_set` (что разрешил
  пир — половина всех вопросов «почему передача встала на N байтах»),
  `transport:ecn_state_updated`, `recovery:loss_timer_expired` и три события
  DPLPMTUD (`mtu_probe_sent` / `mtu_probe_lost` / `mtu_updated`).
* `recovery:packet_lost` и `metrics_updated` шлёт сам `quicloss` — только он
  знает номера пакетов, о которых идёт речь. Указатель на журнал он одалживает
  (`quicloss_set_qlog`), как и метку CID: модуль ничего не должен слоям выше.
