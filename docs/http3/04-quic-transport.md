...
Не проверено вживую: **полное рукопожатие TLS**. Для него нужен клиент,
присылающий настоящий ClientHello, а QUIC-клиента в системе нет
(`curl 8.18` без HTTP3). Мост TLS отдельно проверен в фазе 3 — рукопожатие
между двумя `quictls` в памяти проходит целиком, — но связка «рукопожатие
поверх реальных пакетов» ждёт тестового клиента из `08-testing.md` §2.
