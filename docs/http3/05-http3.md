...
**Осталось:** сквозной прогон — путь запрос→ответ **собран целиком, но ни разу
не проходил через живое соединение**: для этого нужен h3-клиент поверх
`tests/quicclient` (док 08 §2), которого пока нет. Дальше: 103 и 100-continue
через `h3response_informational`, Extended CONNECT (§8), фаза 7 (Alt-Svc,
метрики, свои ключи конфига).
