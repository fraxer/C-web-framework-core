...
планирование по urgency пока намеренно не меняется.

Цена этого решения измерена (`08` §7p, стадия `tests/ci.sh priority`): цикл
наполнения пакета не имеет кванта на поток, поэтому маленький ответ, запрошенный
вторым, ждёт всю большую передачу до конца — 85–112 мс против 0,1 мс, когда тот
же ответ стоит в списке впереди при той же нагрузке. Наш собственный h2 на одном
соединении отдаёт его за 2,8 мс. Заголовок `Priority` запроса не разбирается
вовсе.

Некорректный payload даёт `H3_FRAME_ERROR`, недопустимый request/push ID —
`H3_ID_ERROR`, кадр на request stream — `H3_FRAME_UNEXPECTED`.
