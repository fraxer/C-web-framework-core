...
**Осталось:** транспортный клей — вызвать `h3conn_read`/`h3conn_write` из
`__conn_process` в `quicendpoint.c` и из flush'а tx-очереди, создать `h3conn_t`
на соединении и выбрать h3 по ALPN. Дальше: 103 и 100-continue через
`h3response_informational`, Extended CONNECT (§8), фаза 7 (конфиг, Alt-Svc,
метрики).
