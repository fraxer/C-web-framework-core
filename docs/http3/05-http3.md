...
**Осталось:** `h3_write_filter` — и перед ним решение, которого пока нет:
`quicsendbuf_write()` растёт **без предела** (флоу-контроль там сознательно не
его дело), поэтому «записать тело ответа в поток» для файла в гигабайт означает
гигабайт в памяти. Нужен либо high-water mark на `quicsendbuf` с возобновлением
по ACK'ам (аналог `CWF_EVENT_AGAIN` в h2-фильтре), либо pull-источник, который
транспорт дёргает при сборке пакета. Дальше — подключение dispatch, Extended
CONNECT (§8) и фаза 7.
