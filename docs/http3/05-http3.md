...
**Осталось:** подключение dispatch — вызвать `http_server_dispatch` по
`H3CONN_REQUEST_DONE`, положить `h3stream->response` и завести публикацию
ответов из хендлерных потоков (§6.2, включая inline-вариант под уже взятым
локом); ответы на `H3CONN_REQUEST_REFUSED` (413/431/500); трейлеры и 103 через
`h3response_trailers`/`_informational`; Extended CONNECT (§8); фаза 7.
