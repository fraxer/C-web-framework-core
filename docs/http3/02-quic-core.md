...
**Генерация серверных CID** — 8 байт, все случайные (`RAND_bytes`).

Байт `[0]` первоначально резервировался под индекс воркера — чтобы воркер
узнавал свои соединения по CID. Он так и остался незанятым, и намеренно:
аффинность сделана переносом владения (`09` §2.6), а не разбором CID, потому что
в сервере на потоках чтение байта не окупается (ссылку на соединение всё равно
надо брать под замком, и стоит именно она), а после переноса байт называл бы
прежнего владельца. Байт остаётся зарезервированным ровно под одного
потребителя, которому он был бы нужен, — программу `SK_REUSEPORT` на eBPF,
раздающую датаграммы сокету владельца ещё в ядре.

Требование RFC 9000 §5.1: CID не должен позволять связать соединения между
собой — восемь случайных байт с запасом; индекс воркера, если он когда-нибудь
там появится, утечкой не считается (nginx/quiche делают так).
