...
| RFC 9218 приоритеты | Готово (2026-08-16): `priority` в заголовках запроса и оба типа `PRIORITY_UPDATE`, планирование по urgency в цикле отправки и в раздаче бюджета записи, инкрементальные ответы делят соединение по очереди. Значение вне диапазона игнорируется (§4.1). `u=0` за 64 МБ ответом — 0,1 мс вместо 85; `08` §7q |
