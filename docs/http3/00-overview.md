...
| Ориентир производительности «не хуже 60 % от h2» | `08` §7, §7o | Не выполнен на профиле 57 КБ × `c=100`: 10806–11392 req/s против 24041 у h2 (~45–47 %) после кэша потока в write-фильтре. На коротких ответах с конвейером h3 идёт вровень с h2 и обгоняет его (§7j), так что дыра именно в раздаче тел среднего размера при сотне соединений; крупнейший символ профиля — `memmove` (9,6 %) |
