...
шестнадцать стадий. Открытое — это «шире» и «быстрее», и вот весь список, чтобы
его не приходилось собирать заново по журналу:

| Что | Где разобрано | Состояние |
|---|---|---|
| IPv6-эндпоинт | `01` §7, ADR-5 | Единственная незакрытая функциональная дыра. `src/udp/quicendpoint.c` создаёт и разбирает только `AF_INET`; структуры путей уже на `sockaddr_storage`, то есть работа локализована в эндпоинте и в заполнении `connection_t::remote_ip` |
