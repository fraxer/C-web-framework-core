...
| Ориентир производительности «не хуже 60 % от h2» | `08` §7, §7o, §11 | Не выполнен, но разрыв сокращён вдвое (2026-08-18): чередующийся A/B даёт **9,2–9,8k → 16,0–17,2k req/s** на профиле 57 КБ × `c=100`, на одном сервере h3 15 567 против h2 33 067 — **47 % вместо 29 %**. Четыре причины были измерены, а не угаданы: обход всех потоков ради MTU-пробы на каждой датаграмме, 94 % холостых прогонов цепочки фильтров (бюджет читался раз на проход), пейсер с бурстом, замороженным на начальном окне, и 51 пустой байт в каждой датаграмме из-за «щедрого» резерва заголовка. Осталось: ядро 30 % (GSO-серии 7–8 сегментов из возможных 16+), одна копия в `memmove`, malloc на кадр. На loopback часть разрыва — свойство стенда: TCP там не платит попакетной цены вовсе |
