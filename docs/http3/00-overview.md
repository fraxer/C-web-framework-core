...
- IPv4 и IPv6 endpoint; UDP GSO (`UDP_SEGMENT`) для пакетной отправки;
...
| QUIC v2 (RFC 9369), совместимое согласование версий (RFC 9368) | **Сделано (2026-08-20)**, за флагом `http3_version_2` (по умолчанию выключен) |
...
| Soft reload с сохранением UDP socket/CID | Готово; гейт есть — стадия `tests/ci.sh softreload` шлёт `SIGUSR1` посреди незавершённого запроса и проверяет, что старый CID дочитывается в старом поколении (`08` §8a) |
...
| IPv6 endpoint | Готово (2026-08-17): `ip` в конфиге принимает адрес любой семьи, TCP и UDP слушают её же, ratelimiter считает IPv6-клиента по /64. Сайт на двух семьях — две записи `servers`, различающиеся только `ip`; сокеты v6-only намеренно (`01` §7a) |
| 0-RTT / early data (`http3_early_data`) | Готово, по умолчанию выключено |
| QUIC v2 | Реализовано, включается `http3_version_2` |
| HTTP/3 client, Server Push, Extended CONNECT (RFC 9220) | Вне scope по решению |

### 3.2 Что остаётся открытым (2026-08-17)

Обязательного (то, что RFC предписывает словом MUST) не осталось: группа A в
`09` закрыта, фазы 0–8 закрыты, `h3spec` даёт 49/49, гейт `tests/ci.sh` —
двадцать одну стадию. Функциональных дыр тоже не осталось: последняя, IPv6-эндпоинт,
закрыта 2026-08-17 (`01` §7a). Открытое — это «быстрее» и «по расписанию», и вот
весь список, чтобы его не приходилось собирать заново по журналу:

| Что | Где разобрано | Состояние |
|---|---|---|
| QUIC v2 (RFC 9369), RFC 9368 | `09` §3.2, `08` §17 | **Сделано 2026-08-20.** Всё различие v2 собрано в таблицу `quicversion.{c,h}` (соль, метки `quicv2 *`, ключ Retry, повёрнутые коды типов), версия перестала быть константой в пяти местах. Совместимое согласование (RFC 9368 §2.3) работает: клиент начинает в v1, перечисляет v2 в `version_information` (0x11), сервер отвечает первым флайтом уже в v2 и держит оба набора Initial-ключей, пока клиент не перешёл. Флаг `http3_version_2` по умолчанию **выключен**, и выключенный сервер байт в байт прежний: v2-клиент получает Version Negotiation, предложение v2 игнорируется. Векторы RFC 9369 Приложения A (ключи и Retry-тег) и стадия гейта `version2` из десяти проверок. Кейс `v2` quic-interop-runner прогнан против picoquic (`08` §17g): клиент начинает в v1, сервер отвечает Initial'ом в v2, обе стороны ведут Handshake ровно одной версией — вердикт раннера на этой машине недостижим (нет tshark), проверки его же трасс сделаны `interop/pcap_versions.py`. Именно interop нашёл главный дефект (`08` §17f): клиент вправе прочитать в первом пакете новой версии **только поле Version**, поэтому ServerHello в пакете-объявлении пропадал, а подтверждение на него очищало буфер CRYPTO — рукопожатие умирало по idle timeout при зелёном гейте |
| Ориентир производительности «не хуже 60 % от h2» | `08` §7, §7o, §11, §12, §13, §14, §15, §16 | Не выполнен, но разрыв сокращён вдвое (2026-08-18): чередующийся A/B даёт **9,2–9,8k → 16,0–17,2k req/s** на профиле 57 КБ × `c=100`, на одном сервере h3 15 567 против h2 33 067 — **47 % вместо 29 %**. Четыре причины были измерены, а не угаданы: обход всех потоков ради MTU-пробы на каждой датаграмме, 94 % холостых прогонов цепочки фильтров (бюджет читался раз на проход), пейсер с бурстом, замороженным на начальном окне, и 51 пустой байт в каждой датаграмме из-за «щедрого» резерва заголовка. GSO-серии закрыты 2026-08-19 (`08` §12): «полные» датаграммы отличались друг от друга на 2–4 байта из-за плоского резерва 24 байта под заголовок STREAM-кадра, а серия кончается на первом изменении размера — точный расчёт плюс TX-batch на 128 датаграмм дали **8,1 → 12,3–15,8 сегмента** на том же профиле, `sendmmsg` 40 000 → 12 000, долю ядра в профиле 35,6 % → 30,5 %. 2026-08-19 (`08` §13) доля доведена до **54–57 %** чередующимся A/B (два сервера подняты одновременно, нагрузка по кругу, медианы семи раундов — блочный порядок дважды дал противоположный знак). Там же измерено, чего этот стенд стоит: h2load однопоточен и занят на 76–93 % ядра, тратя 44 мкс CPU на запрос по h3 против 28 по h2 (вывод §13a о «потолке доли около 64 %» **опровергнут** в §16b: этой ценой клиент ограничивает h2, а не h3); сервер при этом тоже занят (загруженный из двух рабочих потоков — 87–93 %). Закрыто: поиск PLPMTU не успевал начаться (окно открывалось через 10 PTO, а PTO при `c=100` — это своя же приёмная очередь), из-за чего 53–88 соединений из 100 всю жизнь слали 1350 байт вместо 1472; окно открывается сразу, датаграмм −8,3 %. Обход списка потоков сокращён вдвое курсором на один ход (`08` §14): **635 → 288 посещений на запрос** при 41 кадре, CPU сервера на запрос 90 → 84 мкс, доля 58 % → **61 %** в этом замере. Итог дня: **56 % → 59 %**. `08` §15: три следующих кандидата из профиля (счётчик неотправленных байт вместо обхода, курсор в проходе h3, пропуск уплотнения) построены и **измерились нулём или минусом** — дальше этим инструментом идти нельзя, оставшиеся позиции по 1–3 % при разрешении A/B около 5 %. `08` §16: клиент из нескольких процессов построен, и он переписал само число. Одиночный `h2load` **теряет датаграммы в собственном сокете** (`RcvbufErrors` ядра 349 за прогон, 0,16 % потерь, визитов по потокам 2632 на запрос вместо 107) — прогон с ненулевым `RcvbufErrors` меряет восстановление после потерь, а не пропускную способность. Разбитый на два процесса клиент даёт h2 сразу +22 %, а h3 — ноль, то есть прежние 47 % и 59 % были **завышены** придушенным плечом h2. Честная доля при незанятом клиенте: **52 % по пропускной способности** (h3 40,7k против h2 78,6k, `workers: 4`) и **55 % по CPU сервера на запрос** (80 против 44 мкс) — два независимых мерила сошлись. Ориентир 60 % не выполнен. Там же закрыт обход списка потоков при закрытом окне перегрузки: 94 % всех визитов приходились на него, ни один такой вызов не давал кадра; счётчик `ctrl_owed` свёл визиты к **40 на запрос — по одному на STREAM-кадр**, при 100 потоках на соединение это +9 % req/s, при 30 — в пределах шума. Ловушка §16d: тот же указатель, добавленный **в начало** `quicstream_t`, стоил 5 % — сдвиг горячих полей структуры дороже обхода, который он убирает. На loopback часть разрыва — свойство стенда: TCP там не платит попакетной цены вовсе |
| quic-interop-runner | `08` §3e, §8a | Сознательно вне гейта: докер и десятки минут — это расписание, а не PR-гейт. Остаточная краснота `handshakeloss`/quiche — свойство связки «этот клиент + этот сценарий», доказано контрольным опытом quiche против самого себя (§3y) |
| Длинный фаззинг | `08` §5 | Критерий «24 часа на цель» не выполнялся ни разу: в гейте 60 с на каждую из восьми целей. Нужен прогон по расписанию, а не в гейте |
| CI как инфраструктура | `08` §8a | `tests/ci.sh` написан и работает откуда угодно, но раннера (GitHub Actions или иного) в репозитории нет — выбор CI не был решением фазы 8 |
| Заголовок `priority` в **ответе** (RFC 9218 §5) | `08` §7q | Не отправляется: сервер не переопределяет свою же срочность, потребителя нет. Приём обоих сигналов от клиента и планирование по ним — готовы |
| qlog | `04` §10, `08` §10 | **Сделано** (2026-08-18): `quicqlog.c` пишет JSON-SEQ (`.sqlog`) для qvis, включается `http3_qlog_dir` / `http3_qlog_connections`, компилируется всегда — гейт рантаймовый, потому что инцидент случается на уже запущенном сервере. Маяки `quicbeacon.h` остаются для микро-хронометража send-пути |
| GRO / ECN / DPLPMTUD в `/metrics` | `08` §10 | **Сделано** (2026-08-18): `recv.gro_*`, `ecn.*` (метки, коды на приёме, CE-реакция, валидация), `pmtu.*` + гистограмма `pmtu_bytes`. Первый же прогон нашёл дефект тестового клиента, из-за которого поиск PLPMTU не завершался никогда |
...
На момент решения сокет-слой был чисто IPv4 (`in_addr_t`, `AF_INET`). QUIC обязан хранить
...
Побочный эффект: h3 стал первым куском проекта с IPv6, и он же вытянул за собой
весь остальной сервер — `in_addr_t` заменён одним типом `ipaddr_t` вместо
обещанного здесь «отдельного пути для IPv6», по причинам в `01` §7a.
...
| 9 | Отложенное: обязательное (закрыто), производительность (вся группа B сделана), опции (0-RTT и QUIC v2 сделаны) — **группа закрыта целиком** | `09` | — | По отдельности, каждый пункт за своим флагом и со своим критерием — см. `09` |
