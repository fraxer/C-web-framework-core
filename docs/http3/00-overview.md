...
двадцать стадий. Функциональных дыр тоже не осталось: последняя, IPv6-эндпоинт,
...
|---|---|---|
| QUIC v2 (RFC 9369), RFC 9368 | `09` §3.2 | Отложено по решению «нет потребителя». Страховка (Version Negotiation) проверена стадией `ci.sh vn` (`08` §8b); непроверенным остаётся то, что клиент после VN доводит рукопожатие по v1 — для этого нужна вторая сторона, умеющая v2 |
