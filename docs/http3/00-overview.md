...
соединения-владельца. Аффинность по CID (номер воркера в первом байте
server-chosen CID) — оптимизация фазы 9, не условие корректности.
...
| 4 | Транспорт: потоки, flow control, ACK, loss detection, NewReno, pacing, закрытие | `04` | XL | Соединение живёт, переносит 100 МБ через один поток, переживает 10 % потерь |
