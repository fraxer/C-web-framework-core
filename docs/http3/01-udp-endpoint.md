...
Проверка версии OpenSSL срабатывает корректно и на этой машине **отклоняет**
сборку с h3 (см. `00-overview.md` §2.1 — `/usr/local` затеняет системный 3.5.5);
ON-путь проверен с временно ослабленным гейтом, поскольку код фазы 0 к OpenSSL
не обращается.
