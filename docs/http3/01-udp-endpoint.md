...
*2026-08-18:* заглушки больше нет — `quicqlog.c` написан (`04` §10, `08` §10), и
`QLOG()` теперь проверка указателя перед вызовом. Сборочного флага у qlog нет
намеренно: инцидент случается на уже запущенном сервере. Модуль получил
единственную зависимость сверх libc — `misc` (ради `log.h`): диагностика,
которая молча не открывает файл, хуже отсутствующей.
...
| `IPV6_V6ONLY`, `IPV6_RECVPKTINFO`, `IPV6_MTU_DISCOVER`, `IPV6_RECVTCLASS` | IPv6-путь (§7). Сокет — v6-only намеренно: dual-stack отдаёт IPv4-адреса как v4-mapped, из-за чего cmsg с локальным адресом становится двусмысленным, а поведение зависит от `net.ipv6.bindv6only` |
...
### 7a. Сделано (2026-08-17): один тип адреса вместо двух путей

План предлагал **два пути**: `remote_ip` для IPv4 и параллельное поле
`remote_addr` для IPv6. Сделано иначе — `in_addr_t` заменён на один тип
`ipaddr_t` (`misc/ipaddr.h`), и причина в том, что «два пути» означали бы два
источника истины в структуре, которую читают ratelimiter, выбор vhost и логи:
каждый такой читатель обязан был бы помнить, какое из полей сегодня настоящее, а
забывший получал бы не ошибку, а адрес `0.0.0.0`, который с чем-нибудь да
совпадёт. `ipaddr_t` — 20 байт (семейство + union адресов) вместо
`sockaddr_storage`, потому что поле лежит в каждом `connection_t`, и порт в него
не входит: порты уже есть рядом (`server_t::port`, `http3.port`,
`connection_t::port`), а вторая копия порта — это второе, что надо
синхронизировать.

Нулевое значение — `AF_UNSPEC`, и это **не** wildcard `0.0.0.0`. Различие
оплачивается сразу: пропущенный `ip` в конфиге ловится как пропущенный, а не
превращается молча в «все интерфейсы».

Работа оказалась не только в эндпоинте. Полный список того, что было привязано к
IPv4, и во что превратилось:

| Место | Было | Стало |
|---|---|---|
| `server_t::ip` | `in_addr_t` из `inet_addr()` | `ipaddr_t` из `ipaddr_parse()`; принимается `1.2.3.4`, `::1` и `[::1]` |
| Проверка конфига | `inet_addr` возвращает `(in_addr_t)-1` и на ошибку, и на валидный `255.255.255.255` | опечатка **отклоняет конфиг** с указанием значения |
| `socket_listen_create` | `(in_addr_t, port)`, всегда `AF_INET` | `(const ipaddr_t*, port)`, семейство из адреса, для IPv6 — `IPV6_V6ONLY` |
| `accept()` | `struct sockaddr` (16 байт) | `sockaddr_storage`; IPv6-адрес пира приезжал **обрезанным пополам** при целом порте |
| `connection_t::ip`/`remote_ip` | `in_addr_t` | `ipaddr_t` |
| Выбор vhost (`httpparser_select_server`, SNI-колбэк) | `server->ip == connection->ip` | `ipaddr_equal()`; сравнение по семейству — то, что не даёт `0.0.0.0` и `::` совпасть |
| Уникальность vhost'ов | ключ `(домен, порт)` | ключ `(адрес, домен, порт)`; иначе один сайт на двух семействах — два одинаковых домена, и конфиг отклонялся |
| Ratelimiter | ключ `in_addr_t` | `ipaddr_client_key()`: весь адрес для v4, префикс **/64** для v6; ключи v4 помечены в старших 32 битах, чтобы не столкнуться с `::/64` (в которой лежит `::1`) |
| `quicendpoint` | `sockaddr_in`, `AF_INET` | `ipaddr_to_sockaddr()`, поиск эндпоинта по `(ipaddr_t, port)` |
| `quicconn_accept` | адрес пира для v6 сообщался как `0`/`0` | обе семьи |
| `tests/quicclient` | `sockaddr_in` | `sockaddr_storage`: иначе IPv6-эндпоинт нечем проверить |

Клиентские пути (`httpclient`, SMTP) остались IPv4 по существу — они резолвят
имя в `struct in_addr` — но проходят через тот же тип, и
`ipaddr_v4_addr()` для IPv6 возвращает 0, а не переинтерпретированные четыре
байта: забывший проверить семейство получает wildcard, который не подключится,
а не правдоподобно неверный адрес.

**Одна семья — один сокет, одна запись vhost'а.** Dual-stack сокет отклонён там
же, где это решил UDP-слой в фазе 1: v4-mapped адреса делают cmsg с локальным
адресом двусмысленным, а `net.ipv6.bindv6only` превращает один и тот же конфиг в
два разных смысла. Сайт на обеих семьях — две записи в `servers`,
различающиеся только `ip`, как две IPv4-записи и раньше.

**Ловушка, которую стоит помнить.** Смена типа поля с `in_addr_t` на структуру
**не** ломает статический инициализатор `.ip = 0x0100007F`: правило исключения
скобок (brace elision) присваивает значение первому подобъекту, то есть
`family`, и код собирается. Три мока в `tests/unit/` собрались, `server->ip`
получил семейство 127, выбор vhost перестал совпадать — и юнит-раннер упал в
segfault на разыменовании `ctx->server`. Компилятор молчит; тесты — нет.

**Проверено.** `tests/h3_ipv6.sh` (стадия `ipv6` в `tests/ci.sh`): h1.1, h2 и h3
на `::1` отдают свой vhost; счётчик `quic.handshakes_completed` на сервере
подтверждает рукопожатие независимо от отчёта клиента; миграция по IPv6;
IPv4-vhost на **том же номере порта** продолжает отдавать свой root по TCP и по
h3 — это и есть проверка на «семейства не слиплись в один эндпоинт»; битый `ip`
отклоняет конфиг. Плюс `h3spec` против `::1` — 49/49. Юнит-тесты типа —
`tests/unit/test_ipaddr.c`.
