...
- **Ciphersuites для QUIC задаются отдельно, а не наследуются от vhost.**
  RFC 9001 §5.3 запрещает `TLS_AES_128_CCM_8_SHA256`: 64-битный тег слишком
  короток для требований QUIC к целостности. Между тем `config.json` этого
  проекта его перечисляет (проверено: OpenSSL 3.5 принимает его в
  `SSL_CTX_set_ciphersuites` без возражений), поэтому QUIC-`SSL` обязан
  выставить свой список — иначе договорившись о CCM_8, соединение нарушит
  спецификацию, а мы этого даже не заметим. Заодно исключаем и
  `TLS_AES_128_CCM_SHA256`: в v1 CCM не поддерживаем (`00` §3).
- `SSL_CTX_set_max_early_data(ctx, 0)` на контексте: 0-RTT поднимается **на
  отдельном SSL** в `quictls_init_server`, когда включён `http3_early_data`
  (`09` §3.1). Там же — session-id context, привязывающий тикет к конфигурации.
- `SSL_CTX_set_quiet_shutdown` и `SSL_set_shutdown` перед `SSL_free`: в QUIC нет
  `close_notify`, а OpenSSL без этого считает каждое соединение оборванным и
  выбрасывает его сессию — тихо ломая и resumption, и 0-RTT (`09` §3.1).
...
- 0-RTT read-ключи выбрасываются после получения первого 1-RTT-пакета (§4.9.3).
  **Только ключи:** 0-RTT и 1-RTT — одно пространство номеров (§12.3), поэтому
  `ack[APP]` и его диапазоны переживают сброс. Реализовано в `__process_packet`
  вместе с `__ack_space()`; подробности — `09` §3.1.
...
## 7a. Что из этой фазы перенесено в фазу 4

Криптослой готов и проверен, но **живого рукопожатия по сети ещё нет**: его
некуда поместить. Приём Initial-пакета создаёт `quicconn_t`, а он появляется в
фазе 4 — эндпоинт по-прежнему считает такие пакеты счётчиком
`initial_dropped_no_tls` и отбрасывает. Соответственно перенесены:

- создание соединения из Initial (`01` §5.2);
- поле `connection_t::transport` и обход `epoll_ctl` (`01` §3.1–3.3) — они нужны
  QUIC-*соединениям*, которых всё ещё нет;
- отправка Retry по порогу нагрузки — сам Retry-пакет и токены готовы, решение
  «слать или нет» принимает соединение;
- сброс ключей уровней (§6) и stateless reset при неудачной расшифровке — это
  состояние соединения.

Критерий «`curl --http3` доходит до Handshake Done» перенесён в фазу 4 и по
другой причине тоже: в системе нет ни одного QUIC-клиента (`curl 8.18` собран
без HTTP3, aioquic/quiche/ngtcp2 отсутствуют). Понадобится либо собрать клиент,
либо контейнер — тот же, что нужен для quic-interop-runner в фазе 8.

## 8. Юнит-тесты фазы

**Статус: сделано.** +183 проверки (100 525 → 100 704), ASan чист, сборка без
флага не изменилась (100 156).

Векторы RFC 9001 Приложения A сходятся: Initial-секреты, ключи, iv, hp-ключ
(A.1), защищённый payload и защищённый заголовок клиентского Initial (A.2),
Retry-тег (A.4). Это самая ценная проверка проекта — самосогласованная ошибка
пережила бы любой round-trip собственного кода, а вектор нет.

Сверх плана добавлен **клиентский `quictls_init_client`** и тест, проводящий
настоящее рукопожатие TLS 1.3 между двумя мостами в одном процессе через
трубу вместо CRYPTO-потоков. Без второй стороны утверждение «шесть колбэков
срабатывают в правильном порядке с правильными уровнями» непроверяемо, а
проверять его на interop поздно. Та же функция станет основой тестового клиента
из `08-testing.md` §2.

### Найденная ошибка: OpenSSL не копирует transport parameters

`SSL_set_quic_tls_transport_params()` **сохраняет указатель**, а не копирует
буфер, и читает его позже — когда расширение попадает в ClientHello/EncryptedExtensions.
Локальный массив в функции инициализации компилируется, проходит round-trip
собственного кодека и читает освобождённый стек в момент рукопожатия.

Симптом был непрозрачным: сервер отвергал расширение с
`custom_ext_parse: bad extension`, что выглядит как ошибка в нашем кодеке
transport parameters — а тот работал безупречно. Диагностировано сравнением с
сырым двусторонним прогоном OpenSSL (он проходил), затем ASan указал точное
место: `stack-use-after-return` на `encoded` в `__init`.

**Важно для отладки:** ASan сообщил об этом только при
`detect_stack_use_after_return=1`. Привычная для профилирования настройка
проекта `=0` (`core/INSTALL.md`) этот класс ошибок маскирует — по этой причине
юнит-тест сначала просто «не сходился», без диагностики. Буфер теперь живёт в
`quictls_t`.
