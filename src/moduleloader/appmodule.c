...
int app_modules_load(const json_token_t* root) {
    ...
    if (!json_is_array(token_modules)) {
        log_error_stderr("app_modules_load: main.modules must be array\n");
        return 0;
    }

    json_it_t it = json_init_it(token_modules);
    if (!it.ok) {
        log_error_stderr("app_modules_load: memory alloc error for main.modules\n");
        return 0;
    }
    ...
    for (; !json_end_it(&it); it = json_next_it(&it)) {
        ...
        if (!json_is_string(token_path)) {
            log_error_stderr("app_modules_load: main.modules items must be strings\n");
            return 0;
        }
        if (json_string_size(token_path) == 0) {
            log_error_stderr("app_modules_load: main.modules items must be not empty\n");
            return 0;
        }

        if (!__app_module_init(json_string(token_path)))
            return 0;
    }
