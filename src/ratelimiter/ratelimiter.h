...
// Bucket для одного клиента
typedef struct ratelimiter_bucket {
    /* Ключ клиента (ipaddr_client_key): весь адрес для IPv4, префикс /64 для
     * IPv6 — один абонент получает целую /64, и лимит по /128 не лимит. */
    uint64_t key;
...
/**
 * Проверка, разрешён ли запрос от данного клиента
 * @param limiter - rate limiter
 * @param ip - адрес клиента (IPv4 или IPv6); NULL всегда разрешён
