#include <unistd.h>
#include <stdlib.h>

#include "cqueue.h"

cqueue_t* cqueue_create() {
    cqueue_t* queue = malloc(sizeof * queue);
    if (queue == NULL) return NULL;

    cqueue_init(queue);

    return queue;
}

void cqueue_init(cqueue_t* queue) {
    if (queue == NULL) return;

    atomic_store(&queue->locked, 0);
    queue->item = NULL;
    queue->last_item = NULL;
    queue->size = 0;
}

void cqueue_free(cqueue_t* queue) {
    cqueue_freecb(queue, NULL);
}

void cqueue_freecb(cqueue_t* queue, void(*free_cb)(void*)) {
    cqueue_clearcb(queue, free_cb);
    free(queue);
}

int cqueue_append(cqueue_t* queue, void* data) {
    if (queue == NULL) return 0;
    if (data == NULL) return 0;

    cqueue_item_t* item = cqueue_item_create(data);
    if (item == NULL) return 0;

    if (queue->last_item != NULL)
        queue->last_item->next = item;

    queue->last_item = item;

    if (queue->item == NULL)
        queue->item = item;

    queue->size++;

    return 1;
}

int cqueue_prepend(cqueue_t* queue, void* data) {
    if (queue == NULL) return 0;
    if (data == NULL) return 0;

    cqueue_item_t* item = cqueue_item_create(data);
    if (item == NULL) return 0;

    if (queue->last_item == NULL)
        queue->last_item = item;

    item->next = queue->item;

    queue->item = item;

    queue->size++;

    return 1;
}

void* cqueue_pop(cqueue_t* queue) {
    if (queue == NULL) return NULL;

    cqueue_item_t* item = queue->item;
    if (item == NULL) return NULL;

    void* data = item->data;
    queue->item = item->next;
    if (queue->item == NULL)
        queue->last_item = NULL;

    cqueue_item_free(item);

    queue->size--;

    return data;
}

int cqueue_empty(cqueue_t* queue) {
    if (queue == NULL) return 1;

    return queue->item == NULL;
}

int cqueue_size(cqueue_t* queue) {
    if (queue == NULL) return 0;

    return queue->size;
}

cqueue_item_t* cqueue_first(cqueue_t* queue) {
    if (queue == NULL) return NULL;

    return queue->item;
}

cqueue_item_t* cqueue_last(cqueue_t* queue) {
    if (queue == NULL) return NULL;

    return queue->last_item;
}

int cqueue_lock(cqueue_t* queue) {
    if (queue == NULL) return 0;

    int expected = 0;
    int desired = 1;

    do {
        expected = 0;
    } while (!atomic_compare_exchange_strong(&queue->locked, &expected, desired));

    return 1;
}

int cqueue_incrementlock(cqueue_t* queue) {
    if (queue == NULL) return 0;

    atomic_fetch_add(&queue->locked, 1);

    return 1;
}

int cqueue_unlock(cqueue_t* queue) {
    if (queue == NULL) return 0;

    atomic_fetch_sub(&queue->locked, 1);

    return 1;
}

void cqueue_clear(cqueue_t* queue) {
    cqueue_clearcb(queue, NULL);
}

void cqueue_clearcb(cqueue_t* queue, void (*free_cb)(void *)) {
    if (queue == NULL) return;

    cqueue_item_t* item = queue->item;
    while (item) {
        cqueue_item_t* next_item = item->next;
        if (free_cb != NULL)
            free_cb(item->data);

        cqueue_item_free(item);
        item = next_item;

        queue->size--;
    }

    queue->item = NULL;
    queue->last_item = NULL;
}

cqueue_item_t* cqueue_item_create(void* data) {
    cqueue_item_t* item = malloc(sizeof * item);
    if (item == NULL) return NULL;

    item->data = data;
    item->next = NULL;

    return item;
}

void cqueue_item_free(cqueue_item_t* item) {
    if (item == NULL) return;

    free(item);
}
