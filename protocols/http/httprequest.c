...
void httprequest_init_payload(httprequest_t*);
...
int httprequest_append_formdata_json(httprequest_t*, const char*, json_doc_t*);
int httprequest_append_formdata_filepath(httprequest_t*, const char*, const char*);
int httprequest_append_formdata_file(httprequest_t*, const char*, file_t*);
int httprequest_append_formdata_file_content(httprequest_t*, const char*, file_content_t*);
int httprequest_set_payload_raw(httprequest_t*, const char*, const size_t, const char*);
int httprequest_set_payload_text(httprequest_t*, const char*);
int httprequest_set_payload_json(httprequest_t*, json_doc_t*);
int httprequest_set_payload_filepath(httprequest_t*, const char*);
int httprequest_set_payload_file(httprequest_t*, const file_t*);
int httprequest_set_payload_file_content(httprequest_t*, const file_content_t*);
...
httprequest_t* httprequest_create(connection_t* connection) {
    httprequest_t* request = httprequest_alloc();
    ...
    request->append_formdata_json = httprequest_append_formdata_json;
    request->append_formdata_filepath = httprequest_append_formdata_filepath;
    request->append_formdata_file = httprequest_append_formdata_file;
    request->append_formdata_file_content = httprequest_append_formdata_file_content;
    request->set_payload_raw = httprequest_set_payload_raw;
    request->set_payload_text = httprequest_set_payload_text;
    request->set_payload_json = httprequest_set_payload_json;
    request->set_payload_filepath = httprequest_set_payload_filepath;
    request->set_payload_file = httprequest_set_payload_file;
    request->set_payload_file_content = httprequest_set_payload_file_content;
    request->base.reset = (void(*)(void*))httprequest_reset;
    request->base.free = (void(*)(void*))httprequest_free;
...
void httprequest_reset(httprequest_t* request) {
    ...
    http_headers_free(request->header_);
    request->header_ = NULL;
    request->last_header = NULL;
...
http_header_t* httprequest_header(httprequest_t* request, const char* key) {
    return httprequest_headern(request, key, strlen(key));
}

http_header_t* httprequest_headern(httprequest_t* request, const char* key, size_t key_length) {
...
const char* httprequest_cookie(httprequest_t* request, const char* key) {
    http_cookie_t* cookie = request->cookie_;
...
char* httprequest_payload(httprequest_t* request) {
    return httprequest_payloadf(request, NULL);
}
...
file_content_t httprequest_payload_file(httprequest_t* request) {
    return httprequest_payload_filef(request, NULL);
}
...
json_doc_t* httprequest_payload_json(httprequest_t* request) {
    return httprequest_payload_jsonf(request, NULL);
}
...
int httprequest_allow_payload(httprequest_t* request) {
...
int httprequest_header_add(httprequest_t* request, const char* key, const char* value) {
    return httprequest_headern_add(request, key, strlen(key), value, strlen(value));
}

int httprequest_headern_add(httprequest_t* request, const char* key, size_t key_length, const char* value, size_t value_length) {
...
size_t httprequest_head_size(httprequest_t* request) {
    size_t size = httprequest_method_length(request->method);
    if (size == 0) return 0;
...
int httprequest_data_append(char* data, size_t* pos, const char* string, size_t length) {
...
httprequest_head_t httprequest_create_head(httprequest_t* request) {
    ...
    if (httprequest_data_append(head.data, &pos, " ", 1) == -1) return head;
    if (httprequest_data_append(head.data, &pos, request->uri, request->uri_length) == -1) return head;
    if (httprequest_data_append(head.data, &pos, " HTTP/1.1\r\n", 11) == -1) return head;

    http_header_t* header = request->header_;
    while (header) {
        if (httprequest_data_append(head.data, &pos, header->key, header->key_length) == -1) return head;
        if (httprequest_data_append(head.data, &pos, ": ", 2) == -1) return head;
        if (httprequest_data_append(head.data, &pos, header->value, header->value_length) == -1) return head;
        if (httprequest_data_append(head.data, &pos, "\r\n", 2) == -1) return head;
...
int httprequest_append_urlencoded(httprequest_t* request, const char* key, const char* value) {
...
int httprequest_create_payload_boundary(http_payload_t* payload, const size_t boundary_size) {
...
int httprequest_insert_content_disposition_file_header_payload(http_payload_t* payload, const char* key, const char* filename) {
...
int httprequest_insert_header_payload(http_payload_t* payload, const char* template, const char* key) {
...
int httprequest_add_header_multipart(httprequest_t* request) {
...
int httprequest_append_formdata_raw(httprequest_t* request, const char* key, const char* value, const char* mimetype) {
    http_payload_t* payload = &request->payload_;
    file_t* file = &payload->file;
    ...
    if (!httprequest_insert_header_payload(payload, "Content-Disposition: form-data; name=\"%s\"\r\n", key))
        return 0;
    if (!httprequest_insert_header_payload(payload, "Content-Type: %s\r\n\r\n", mimetype))
        return 0;
...
int httprequest_append_formdata_text(httprequest_t* request, const char* key, const char* value) {
    return httprequest_append_formdata_raw(request, key, value, "text/plain");
}

int httprequest_append_formdata_json(httprequest_t* request, const char* key, json_doc_t* document) {
    return httprequest_append_formdata_raw(request, key, json_stringify(document), "application/json");
}

int httprequest_append_formdata_filepath(httprequest_t* request, const char* key, const char* filepath) {
    file_t file = file_open(filepath, O_RDONLY);
    if (!file.ok) return 0;

    const int result = httprequest_append_formdata_file(request, key, &file);
    file.close(&file);

    return result;
}

int httprequest_append_formdata_file(httprequest_t* request, const char* key, file_t* file) {
    file_content_t file_content = file_content_create(file->fd, file->name, 0, file->size);

    return httprequest_append_formdata_file_content(request, key, &file_content);
}

int httprequest_append_formdata_file_content(httprequest_t* request, const char* key, file_content_t* file_content) {
    http_payload_t* payload = &request->payload_;
    file_t* file = &payload->file;
...
int httprequest_set_payload_raw(httprequest_t* request, const char* value, const size_t value_size, const char* mimetype) {
    http_payload_t* payload = &request->payload_;
    file_t* file = &payload->file;
...
int httprequest_set_payload_text(httprequest_t* request, const char* value) {
    return httprequest_set_payload_raw(request, value, strlen(value), "text/plain");
}
...
int httprequest_set_payload_filepath(httprequest_t* request, const char* filepath) {
    ...
    const int result = httprequest_set_payload_file(request, &file);
    file.close(&file);

    return result;
}

int httprequest_set_payload_file(httprequest_t* request, const file_t* file) {
    file_content_t file_content = file_content_create(file->fd, file->name, 0, file->size);

    return httprequest_set_payload_file_content(request, &file_content);
}

int httprequest_set_payload_file_content(httprequest_t* request, const file_content_t* file_content) {
    http_payload_t* payload = &request->payload_;
    file_t* file = &payload->file;
