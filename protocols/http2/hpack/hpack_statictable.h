...
/* The lengths are carried alongside the strings: the encoder scans the whole
 * table for every header it writes, and strlen() on each entry made that scan
 * the largest single item in the response profile. */
typedef struct {
    const char* name;
    size_t      name_len;
    const char* value;
