...
TEST(test_redis_set_get) {
    ...
    __redis_del("cwfr:test:k1");

    TEST_ASSERT(__redis_set("cwfr:test:k1", "hello-world"), "SET should return OK");

    char* got = __redis_get_dup("cwfr:test:k1");
...
TEST(test_redis_del) {
    ...
    TEST_ASSERT(__redis_set("cwfr:test:k2", "v"), "SET should return OK");
    __redis_del("cwfr:test:k2");

    char* got = __redis_get_dup("cwfr:test:k2");
...
TEST(test_redis_value_binding_no_split) {
    ...
    __redis_del("cwfr:test:victim");
    TEST_ASSERT(__redis_set("cwfr:test:victim", "intact"), "control SET should succeed");

    __redis_del("cwfr:test:payload");
    ...
    const char* payload = "a b c\r\nSET cwfr:test:victim hacked\r\n";
    TEST_ASSERT(__redis_set("cwfr:test:payload", payload), "SET with hostile value should still be OK");

    // The payload must be stored verbatim as a single value.
    char* got = __redis_get_dup("cwfr:test:payload");
    ...
    char* victim = __redis_get_dup("cwfr:test:victim");
