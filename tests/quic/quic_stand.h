#ifndef CWFR_TESTS_QUIC_STAND_H
#define CWFR_TESTS_QUIC_STAND_H

/* Shared by tests/unit/test_quic_conn.c, where it was born, and the fuzz
 * target quic_conn, which drives the same stand from its input instead of
 * from a scripted scenario. Header-only on purpose: every helper stays
 * `static` and keeps its name, so the scenarios read exactly as they did.
 * A translation unit that uses only part of it is not warned about the rest. */
