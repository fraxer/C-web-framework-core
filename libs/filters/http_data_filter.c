...
static bufo_t* body_next_chunk_data(http1response_t* response, int** ok);
static bufo_t* file_next_chunk_data(http1response_t* response, int** ok);
static bufo_t* next_chunk_data(http1response_t* response, int* ok);
...
int __body(http1response_t* response, bufo_t* parent_buf) {
    ...
    while (1) {
        ...
        buf = next_chunk_data(response, &ok);
        if (!ok) /* alloc error */
            return CWF_ERROR;
...
bufo_t* body_next_chunk_data(http1response_t* response, int** ok) {
    **ok = 1;
...
bufo_t* next_chunk_data(http1response_t* response, int* ok) {
    if (response->file_.fd > -1)
        return file_next_chunk_data(response, &ok);

    return body_next_chunk_data(response, &ok);
}
