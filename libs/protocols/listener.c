...
static int __listener_create_connection(connection_s_t*, server_t*);
static void __listener_connection_set_hooks(connection_s_t*);
static int __listener_after_read_request(connection_s_t*);
static int __listener_after_write_request(connection_s_t*);
...
static int __listener_queue_pop(connection_s_t*);

void listener_read(connection_s_t* socket_connection, char* buffer, size_t buffer_size) {
    (void)buffer;
    (void)buffer_size;
    connection_s_t* connection = connection_s_create(socket_connection);
    if (connection == NULL) return;

    __listener_create_connection(connection, socket_connection->server);
}

int __listener_create_connection(connection_s_t* connection, server_t* server) {
    server_t* s = server;
    while (s) {
        if (s->ip == connection->base.ip && s->port == connection->base.port) {
            connection->server = s;
            break;
        }
        s = s->next;
    ...
    if (s->openssl) {
        connection->base.ssl_ctx = s->openssl->ctx;
...
void __listener_connection_set_hooks(connection_s_t* connection) {
...
int __listener_after_read_request(connection_s_t* connection) {
    return connection->api->control_mod(connection, MPXOUT | MPXRDHUP);
}

int __listener_after_write_request(connection_s_t* connection) {
    if (connection->base.keepalive_enabled == 0) {
...
int __listener_queue_pop(connection_s_t* connection) {
    return connection->api->control_mod(connection, MPXOUT | MPXRDHUP);
}

int listener_connection_close(connection_s_t* connection) {
