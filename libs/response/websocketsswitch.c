...
void switch_to_websockets(httpctx_t* ctx) {
    ...
    connection_s_t* server_connection = ctx->response->connection;
    server_connection->switch_to_protocol = protmgr_set_websockets_default;
    ...
    if (ws_protocol != NULL && strcmp(ws_protocol->value, "resource") == 0) {
        ...
        server_connection->switch_to_protocol = protmgr_set_websockets_resource;
    }

    ctx->response->status_code = 101;
    server_connection->base.keepalive = 1;
}
