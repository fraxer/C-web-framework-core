...
static str_t* __escape_identifier_part(void* connection, const char* start, size_t len);
...
str_t* __escape_identifier(void* connection, const char* str) {
    ...
    while (1) {
        ...
        if (c == '`') {
            // Handle escaped backticks `` inside quoted identifier
            if (in_quotes && *(p + 1) == '`') {
                p += 2;
                continue;
            }
        ...
        if ((c == '.' && !in_quotes) || c == '\0') {
            ...
            str_t* escaped_part = __escape_identifier_part(connection, part_start, part_len);
...
char* __compile_select(void* connection, const char* table, array_t* columns, array_t* where) {
    ...
    for (size_t i = 0; i < array_size(columns); i++) {
        ...
        if (__is_raw_sql(column_name)) {
            ...
            str_t* escaped_col = __escape_identifier(connection, column_name);
            if (escaped_col == NULL)
                goto failed;
